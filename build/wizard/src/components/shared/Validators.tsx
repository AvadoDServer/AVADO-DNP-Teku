...
export const abbreviatePublicKey = (key: string) => (
  <abbr title={key}>{key?.substring(0, 10) + "…"}</abbr>
);

export const createBeaconchainUrl = (
  ...
  return (
    <a
      href={beaconChainBaseUrl + validatorPubkey}
      target="_blank"
      rel="noopener noreferrer"
    >
      {text ? text : validatorPubkey}
    </a>
  );
};
...
const Validators = ({ settings, api, readonly = false }: Props) => {
  ...
  const [feeRecipients, setFeeRecipients] =
    React.useState<feeRecipientType[]>();

  const [configuringfeeRecipient, setConfiguringfeeRecipient] =
    React.useState<ConfiguringfeeRecipient | null>();
  ...
  React.useEffect(() => {
    const fetchFeeRecipient = async (pubKey: string) => {
      ...
      return api.get(
        `/keymanager/eth/v1/validator/${pubKey}/feerecipient`,
        (res) => {
          if (res.status === 200) {
            // console.log(res)
            ...
            if (
              ...
              return result(
                settings?.validators_proposer_default_fee_recipient,
              );
          } else {
        ...
        (err) => {
          console.log(
            "Error in validators_proposer_default_fee_recipient",
            err,
          );
          return result(settings?.validators_proposer_default_fee_recipient);
        },
    ...
    const getValidatorData = async (pubKey: string): Promise<ValidatorData> => {
      ...
      return await api.get(
        `/rest/eth/v1/beacon/states/head/validators/${pubKey}`,
        (res) => {
          // console.dir(res);
    ...
    if (validators) {
      ...
      Promise.all(validators.map((pubKey) => fetchFeeRecipient(pubKey))).then(
        (result) => setFeeRecipients(result),
      );
    }
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "pending_initialized":
        return "is-info"; // When the first deposit is processed, but not enough funds are available (or not yet the end of the first epoch) to get validator into the activation queue.
      case "pending_queued":
        return "is-info"; // When validator is waiting to get activated, and have enough funds etc. while in the queue, validator activation epoch keeps changing until it gets to the front and make it through (finalization is a requirement here too).
      case "active_ongoing":
        return "is-success"; // When validator must be attesting, and have not initiated any exit.
      case "active_exiting":
        return "is-warning"; // When validator is still active, but filed a voluntary request to exit.
      case "active_slashed":
        return "is-danger"; // When validator is still active, but have a slashed status and is scheduled to exit.
      case "exited_unslashed":
        return "is-info"; // When validator has reached reguler exit epoch, not being slashed, and doesn't have to attest any more, but cannot withdraw yet.
      case "exited_slashed":
        return "is-danger"; // When validator has reached reguler exit epoch, but was slashed, have to wait for a longer withdrawal period.
      case "withdrawal_possible":
        return "is-info"; // After validator has exited, a while later is permitted to move funds, and is truly out of the system.
      case "withdrawal_done":
        return "is-info"; // (not possible in phase0, except slashing full balance)// actually having moved funds away
      default:
        return "";
    }
  };
  ...
  const canExit = (validator: ValidatorData) =>
    validator.status === "active_ongoing";

  const getFeeRecipient = (feeRecipients: feeRecipientType[], pubkey: string) =>
    feeRecipients.find((x) => x.pubKey === pubkey)?.recipient ?? "todo";
      ...
      <div className="container has-text-centered ">
        <div className="columns is-vcentered">
          <div className="column">
            ...
            {validators && validators.length < 1 && (
              <>
                <div className="content">
                  ...
                  <p>
                    Click the "Add validator" widget to import your validator
                    keys.
                  </p>
                ...
                <>
                  ...
                  <div className="notification is-success">
                    {beaconchainUrl(
                      "/dashboard?validators=" +
                        validatorData.map((v) => v.index).join(","),
                      <>
                        Beacon Chain Validator DashBoard{" "}
                        <FontAwesomeIcon
                          className="icon"
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {validatorData
                        .sort(
                          (v1, v2) => parseInt(v1.index) - parseInt(v2.index),
                        )
                        .map((validator, i) => (
                          <tr key={`validator-${i}`}>
                            <td>
                              {beaconchainUrl(
                                "/validator/" + validator.validator.pubkey,
                            ...
                            <td>
                              {beaconchainUrl(
                                "/validator/" + validator.validator.pubkey,
                                abbreviatePublicKey(validator.validator.pubkey),
                              )}
                            </td>
                            <td>
                              {(
                                parseFloat(validator.balance) / 1000000000.0
                              ).toFixed(4)}
                            </td>
                            <td>
                              {(
                            ...
                            <td>
                              <span
                                className={
                                  "tag " + getStatusColor(validator.status)
                                }
                              >
                                {validator.status}
                              </span>
                            ...
                            {!readonly && (
                              <td className="has-text-left">
                                <button
                                  className="button is-text has-text-grey-light"
                                  name="delete"
                                  onClick={() =>
                                    askConfirmationRemoveValidator(
                                      validator.validator.pubkey,
                                    )
                                  }
                                >
                      ...
                      <tr>
                        <td colSpan={4}>
                          <a
                            href="http://teku.my.ava.do:9999/backup"
                            className="button"
                          >
                            Download backup of my staking keys
                          </a>
                        </td>
                      </tr>
                    </tbody>
            ...
            {validators && !readonly && (
              <AddValidator updateValidators={updateValidators} api={api} />
            )}
          </div>
        </div>
      </div>
