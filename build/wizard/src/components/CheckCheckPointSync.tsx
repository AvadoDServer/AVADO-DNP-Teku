...
const CheckCheckPointSync = ({ restApi }: Props) => {
    ...
    const [beaconChainStateRoot, setBeaconChainStateRoot] = React.useState<responseType>();
    ...
    React.useEffect(() => {
        ...
        const interval = setInterval(() => {
            getFinalizedState();
        }, 60 * 1000); // 5 seconds refresh
        return () => clearInterval(interval);
    }, [restApi]);



    // Get state root from blockcha.in
    React.useEffect(() => {
        if (finalizedState) {

            const slot = finalizedState.data.header.message.slot

            // fetch(`https://beaconcha.in/block/${slot}`, {
            if (false) {

                fetch(`https://cors-anywhere.herokuapp.com/https://beaconstate.info/checkpointz/v1/beacon/slots/${slot}`, {
                    headers: {
            ...
            // https://beaconstate.ethstaker.cc/checkpointz/v1/beacon/slots
            const url = `https://cors-anywhere.herokuapp.com/https://beaconstate.info/checkpointz/v1/beacon/slots/${slot}`
            // const url = `https://beaconcha.in/block/${slot}`
