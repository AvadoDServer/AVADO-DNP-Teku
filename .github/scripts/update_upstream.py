#!/usr/bin/env python3
"""
Script to automatically update Teku upstream version and package version.
Checks ConsenSys/teku for the latest release and updates configuration files accordingly.
"""
...
def get_latest_teku_release():
    """Fetch the latest release tag from ConsenSys/teku repository."""
    url = "https://api.github.com/repos/ConsenSys/teku/releases/latest"
    
    # Create request with proper headers
    request = urllib.request.Request(url)
    request.add_header('User-Agent', 'AVADO-DNP-Teku-Update-Bot')
...
def increment_patch_version(version):
    """Increment the patch version number (e.g., 0.0.65 -> 0.0.66)."""
    parts = version.split('.')
    if len(parts) != 3:
...
def update_docker_compose(file_path, new_version, new_upstream):
    """Update image tag and TEKU_VERSION in docker-compose.yml."""
    with open(file_path, 'r') as f:
        content = f.read()
    ...
    for line in lines:
        ...
        # Update TEKU_VERSION
            ...
            updated_lines.append(f"{indent}TEKU_VERSION: {new_upstream}")
        else:
...
def main():
    ...
    # Get latest Teku release
    latest_teku_version = get_latest_teku_release()
    print(f"Latest Teku version: {latest_teku_version}")
    ...
    # Compare versions
    if latest_teku_version == current_upstream:
    ...
    update_dappnode_package(dappnode_package_path, new_package_version, latest_teku_version)
